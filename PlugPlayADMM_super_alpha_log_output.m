function [out, history] = PlugPlayADMM_super_alpha_log_output(y,h,K,lambda,method,opts,gt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% out = PlugPlayADMM_super_alpha_log_output(y,h,K,lambda,method,opts)
%
...
if opts.print==true
    ...
    fprintf('Denoiser = %s \n\n', method);
    fprintf('itr \t ||x-xold|| \t ||v-vold|| \t ||u-uold|| \t PSNR (dB) \n');
end
...
while(residual>tol&&itr<=max_itr)
    ...
    alpha  = min(1, sqrt(lambda/rho));
    v      = (1-alpha)*vtilde + alpha*denoise(vtilde,sigma_0);
    ...
    current_psnr = psnr(x, gt);

    if opts.print==true
        fprintf('%3g \t %3.5e \t %3.5e \t %3.5e \t %3.2f \n', itr, residualx, residualv, residualu, current_psnr);
    end

    history(itr) = current_psnr;
